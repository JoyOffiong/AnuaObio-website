...
const lessons = [
  ...
  { img: Bible, title: "Remember your Spiritual Heritage",by:"Min. E.O. Akpan", date: "Jan. 10, 2025", bibleTexts: "2 Tim 1:3; Act 9:15", link: "./teaching/spiritual-heritage" },
];
function Sermon({}: Props) {
 return (
    ...
    >
      ...
      <div className=" md:px-16 px-4 py-16">
      <div className=" text-center md:text-left">
        <p className="font-bold md:text-4xl text-3xl md:ml-48 mb-10 ">Lessons & Sermons</p>
      </div>
        <div className="">
          {lessons.map((lesson, index) => {
            const { img, title, date, bibleTexts , by, link} = lesson;
            return (
              <div key={index} className="mt-6 w-11/12 hover:scale-110 duration-200 transition-all md:w-8/12 mx-auto items-center gap-4">
                {/* Image */}
                <Link href={link}>
                <div className="flex flex-col md:flex-col gap-4  lg:flex-row my-10">
                  <div className=" rounded-2xl">

                  <Image src={img} alt="churchImage" width={300} height={50} className="h-[150px] w-[250px] rounded-2xl max-w-[100%] object-cover" />

                  </div>

                  {/* Texts */}
                  <div className="space-y-4">
                    <p className="md:text-lg font-semibold uppercase">{title}</p>
                    <em className="md:w-2/3 w-full pt-4 text-md font-normal text-[#888888]">{bibleTexts}</em>
                    <p className="md:w-1/2 w-full text-base font-normal text-[#888888]"> by {by}</p>
                    <p className="md:w-1/2 w-full text-base font-normal text-[#888888]">{date}</p>
